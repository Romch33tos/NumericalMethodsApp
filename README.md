...
<img width="600" alt="OlympiadSorting" src="https://github.com/user-attachments/assets/b8efee23-32aa-4b55-9a07-b983ef6c444b" />

## 📗 Вычисление определенного интеграла

### Принцип работы

Численное интегрирование — это приближённое вычисление значения определённого интеграла ∫<sub>a</sub><sup>b</sup> f(x) dx, когда нахождение точного аналитического решения затруднено или невозможно. Методы основаны на аппроксимации площади под кривой y = f(x) на отрезке [a, b] с помощью простых геометрических фигур (прямоугольников, трапеций, парабол).
...
**Алгоритм:**
1.  Отрезок интегрирования [a, b] разбивается на `n` равных частей длиной `h = (b - a) / n`.
2.  В каждом частичном отрезке [x<sub>i-1</sub>, x<sub>i</sub>] значение функции принимается равным f(x<sub>i-1</sub>) — значению в левой границе.
3.  Приближённое значение интеграла вычисляется по формуле:
    `I ≈ h * Σ f(x_{i-1})`, где суммирование ведется от i=1 до n.
...
**Алгоритм:**
1.  Отрезок интегрирования [a, b] разбивается на `n` равных частей длиной `h = (b - a) / n`.
2.  В каждом частичном отрезке [x<sub>i-1</sub>, x<sub>i</sub>] значение функции принимается равным f(x<sub>i</sub>) — значению в правой границе.
3.  Приближённое значение интеграла вычисляется по формуле:
    `I ≈ h * Σ f(x_i)`, где суммирование ведется от i=1 до n.
...
**Алгоритм:**
1.  Отрезок интегрирования [a, b] разбивается на `n` равных частей длиной `h = (b - a) / n`.
2.  В каждом частичном отрезке [x<sub>i-1</sub>, x<sub>i</sub>] значение функции вычисляется в средней точке: `x_{i-1/2} = (x_{i-1} + x_i) / 2`.
3.  Приближённое значение интеграла вычисляется по формуле:
    `I ≈ h * Σ f(x_{i-1/2})`, где суммирование ведется от i=1 до n.
...
**Алгоритм:**
1.  Отрезок интегрирования [a, b] разбивается на `n` равных частей длиной `h = (b - a) / n`.
2.  На каждом частичном отрезке [x<sub>i-1</sub>, x<sub>i</sub>] функция аппроксимируется линейной функцией (прямой, проходящей через точки (x<sub>i-1</sub>, f(x<sub>i-1</sub>)) и (x<sub>i</sub>, f(x<sub>i</sub>))).
3.  Приближённое значение интеграла вычисляется по формуле:
    `I ≈ (h / 2) * [ f(a) + f(b) + 2 * Σ f(x_i) ]`, где суммирование ведется от i=1 до n-1.
...
**Алгоритм:**
1.  Отрезок интегрирования [a, b] разбивается на **чётное** число `n` равных частей длиной `h = (b - a) / n`.
2.  На каждом сдвоенном отрезке [x<sub>2i-2</sub>, x<sub>2i</sub>] функция аппроксимируется квадратичной параболой, проходящей через три точки: (x<sub>2i-2</sub>, f(x<sub>2i-2</sub>)), (x<sub>2i-1</sub>, f(x<sub>2i-1</sub>)), (x<sub>2i</sub>, f(x<sub>2i</sub>)).
3.  Приближённое значение интеграла вычисляется по формуле:
    `I ≈ (h / 3) * [ f(a) + f(b) + 4 * Σ f(x_{2i-1}) + 2 * Σ f(x_{2i}) ]`,
    где первое суммирование ведется от i=1 до n/2, а второе — от i=1 до n/2 - 1.

**Точность:** Имеет более высокий порядок точности по сравнению с предыдущими методами, особенно для гладких функций. Точность метода Симпсона пропорциональна h<sup>4</sup>, тогда как у метода трапеций — h<sup>2</sup>, а у методов прямоугольников — h.
