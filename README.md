...
- **Справочная информация:** встроенная справка по каждому методу с краткой теорией и рекомендациями по использованию.
- **Графическое представление:** наглядная визуализация работы методов с использованием библиотеки **OxyPlot** и графических элементов **WPF**. 
- **Гибкий ввод данных:** парсинг сложных математических функций прямо из строки ввода с помощью **NCalc** с поддержкой стандартных математических операторов и функций.
- **Валидация и очистка данных:** проверка входных данных на корректность и возможность автоматической очистки полей ввода.
-  **Работа с внешними данными:**
    *   **Импорт:** загрузка данных для методов решения СЛАУ, МНК и олимпиадных сортировок из CSV-файлов или напрямую из **Google Таблиц** (по URL).
    *   **Экспорт:** сохранение результатов расчётов решения СЛАУ в CSV-файл.
...
- **Ключевые библиотеки:** OxyPlot (визуализация), NCalc (парсинг выражений)
...
Теоретический метод, основанный на использовании определителей матриц. Если главный определитель системы Δ отличен от нуля, то решение находится по формулам: `x_i = Δ_i / Δ`, где `Δ_i` — определитель матрицы, полученной заменой i-го столбца основной матрицы на столбец свободных членов.
...
**Принцип работы:** самый простой алгоритм сортировки, который многократно проходит через массив, сравнивая соседние элементы и меняя их местами, если они находятся в неправильном порядке. Проходы повторяются до тех пор, пока массив не будет полностью отсортирован.
...
**Принцип работы:** алгоритм строит отсортированную последовательность, по одному элементу за раз, вставляя каждый новый элемент в правильную позицию среди уже отсортированных элементов.
...
**Принцип работы:** улучшенная версия пузырьковой сортировки, которая проходит по массиву в обоих направлениях - сначала слева направо, затем справа налево.
...
**Принцип работы:** эффективный алгоритм сортировки, использующий стратегию "разделяй и властвуй". Выбирается опорный элемент, массив разбивается на две части: элементы меньше опорного и элементы больше опорного, затем рекурсивно сортируются обе части.
...
**Принцип работы:** шуточный алгоритм сортировки, который заключается в случайной перестановке элементов массива до тех пор, пока массив не окажется отсортированным.
...
Метод покоординатного спуска — это итерационный алгоритм оптимизации, используемый для нахождения локального минимума функции двух переменных. Основная идея метода заключается в последовательной минимизации функции по каждой координате в отдельности при фиксации остальных переменных.

**Алгоритм для поиска минимума функции f(x, y):**

1. Задаётся начальное приближение `(x₀, y₀)`, шаг поиска `h` и точность `ε`.
2. На каждой итерации выполняются следующие шаги:
   - **Оптимизация по x:** при фиксированном значении `y` выполняется поиск оптимального `x` на отрезке `[xₖ - R, xₖ + R]`, где `R` — диапазон поиска.
   - **Оптимизация по y:** при найденном значении `x` выполняется поиск оптимального `y` на отрезке `[yₖ - R, yₖ + R]`.
3. Поиск на каждом направлении выполняется перебором значений с шагом `h` и выбором точки с минимальным значением функции.
4. Процесс повторяется до тех пор, пока изменение по обеим координатам не станет меньше заданной точности `ε`:
   - `|xₖ₊₁ - xₖ| < ε`
   - `|yₖ₊₁ - yₖ| < ε`
